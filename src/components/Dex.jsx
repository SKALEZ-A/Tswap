...
const Dex = ({ coins }) => {
  ...
  useEffect(() => {
    const fetchPools = async () => {
      try {
        ...
        // Since DeDust SDK is not available in the browser context, use hardcoded pools
        // This avoids the "DeDust SDK not available" error
        const mockPools = [
          {
            name: "TON/NOT Pool",
            symbol: "LP-TON-NOT",
          ...
          },
          {
            ...
            assets: [
              ...
              {
                address: "EQB-MPwrd1G6WKNkLz_VnV7-yWQQvB2i-RoVxTnv_lQJjNxB",
                metadata: {
                  name: "Bolt",
                  symbol: "BOLT",
        ...
        setPools(mockPools);
        console.log('Using mock pool data since DeDust SDK is not available');
      } catch (error) {
  ...
  // Update filteredCoins when coins prop changes
  useEffect(() => {
    if (coins) {
      console.log("Coins prop updated:", coins.length);
      setFilteredCoins(coins);
    }
  }, [coins]);
  ...
  const getExpectedSwapAmount = async (fromAddress, toAddress, amount) => {
    try {
      ...
      if (!amount || isNaN(parseFloat(amount)) || parseFloat(amount) <= 0) {
        console.log('Invalid amount for swap calculation');
        return 0;
      }
      ...
      const rate = getHardcodedRate(fromAddress, toAddress);
      const expectedOutput = rate * parseFloat(amount);
      setPriceImpact(0.5);
